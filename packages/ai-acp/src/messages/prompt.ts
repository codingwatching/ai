...
export interface BuiltAcpPrompt {
  prompt: string
  /** Harness session id to resume, when the caller threaded one through. */
  resume?: string
}
...
export function buildAcpPrompt(
  ...
  if (!lastUserText) {
    throw new Error(
      `${harnessName} adapter requires a trailing user message with text content.`,
    )
  }
